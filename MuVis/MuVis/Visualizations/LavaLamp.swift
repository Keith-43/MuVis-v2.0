...
struct LavaLamp: View {
    ...
    var body: some View {
        ...
        GeometryReader { geometry in
            ...
            .background( settings.optionOn ?
                         backgroundColor :
                         Color( hue: Double(bckgrndHue360)/360.0,
                                saturation: 1.0,
                                brightness: settings.selectedColorScheme == .dark ?  0.5 : 1.0 ))
    ...
    func updateParameters() {
        ...
        for oct in 0 ..< 6 {
            ...
            for peakNum in 0 ..< peakCount {                                                // peakCount = 16
                if (audioManager.peakBinNumbers[peakNum] >= noteProc.octBottomBin[oct] &&
                    audioManager.peakBinNumbers[peakNum] <= noteProc.octTopBin[oct]) {
                    octEnergy[oct] += 0.2 * audioManager.peakAmps[peakNum]
                }
            }
