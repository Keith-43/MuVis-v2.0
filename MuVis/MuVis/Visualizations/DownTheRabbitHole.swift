...
/// 3. The most recent muSpectrum is rendered on the innermost 8 ellipses (to visually emphasize the newest audio).
/// 4. The next 47 muSpectra are read from this buffer memory and rendered along the outermost 47 of the 8+47=55 concentric ellipses.
///   Each muSpectrum is rendered clockwise starting at the twelve o'clock position.
///
...
struct DownTheRabbitHole: View {
    ...
    var body: some View {
        ...
        Canvas { context, size in
            ...
            DownTheRabbitHole.colorIndex = (DownTheRabbitHole.colorIndex >= colorSize) ? 0 : DownTheRabbitHole.colorIndex + 1
            let startHue360: Int = Int( 360.0 * Double(DownTheRabbitHole.colorIndex) / Double(colorSize) )
...
// ---------------------------------------------------------------------------------------------------------------------
            ...
            for ellipseNum in (0 ..< ellipseCount).reversed()  {       //  0 <= ellipseNum < 64
                let tempHue: Double = (ellipseNum < innermostEllipseCount) ? Double(startHue360) / 360.0 : DownTheRabbitHole.hueOld[ellipseNum-1]
                DownTheRabbitHole.hueOld[ellipseNum] = tempHue
            }
...
// ---------------------------------------------------------------------------------------------------------------------
            for ellipseNum in 0 ..< ellipseCount  {       //  0 <= ellipseNum < 55
                ...
                let histOffset: Int = (newestHist - hist) * sixOctPointCount    // use this for outward flowing spectra

                let hue: Double = ( ellipseNum < innermostEllipseCount) ?
                                    Double(startHue360) / 360.0 :
                                    DownTheRabbitHole.hueOld[ellipseNum-1]      // 0.0 <= hue < 1.0
