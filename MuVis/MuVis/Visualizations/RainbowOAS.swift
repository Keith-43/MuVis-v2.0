...
/// current muSpectrum, it also renders the most-recent 48 muSpectra history - so it shows how the envelope of each note varies with time.
...
/// 41,472 data points.  We use two for() loops.  The outer loop counts through the 6 octaves.  The inner loop counts through the 48 spectra stored in the
            ...
            for oct in octaves {                        // oct = 0, 1, 2, 5, 4, 3
                ...
                for lineNum in (0 ..< lineCount) {   // lineNum = 0, 1, 2, 3 ... 45, 46, 47
                    ...
                    for point in 1 ..< pointsPerOctave {
                        ...
                        magY = 0.6 * Double(audioManager.muSpecHistory[tempIndex]) * lineRampDown * rowHeight
