            ...
            for ellipseNum in 0 ..< ellipseCount {       //  0 <= ellipseNum < 48
                ...
                for oct in 0 ..< octaveCount {     //  0 <= oct < 6
                    ...
                    if(oct==0) {            // oct=0  theta=0  point=0
                        ...
                        mag = 0.3 * Double(audioManager.muSpecHistory[histOffset])
                        let startEllipseY = Y0 - ellipseRampDown * B  * Double( cos(2.0 * Double.pi * startEllipseTheta ) ) - (mag * Y0 * ellipseRampDown)
                    ...
                    for point in 0 ..< pointsPerOctave {    // 12 * 12 = 144 = number of points per octave
                        ...
                        mag = 0.3 * Double(audioManager.muSpecHistory[tempIndex])  // index out of range  26784=histOffset
